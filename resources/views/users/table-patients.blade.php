    ...
    <tbody>
      @forelse($patients as $patientUser)
        @forelse($patientUser->patient as $p)
          <tr>
            <td>{{ $patientUser->id }}</td>
            <td class="d-flex align-items-center">
              <img src="{{ asset('storage/' . ($p->pic ?? 'image.png')) }}" class="rounded-circle me-2" width="50" height="50" alt="Avatar"/>
...
@foreach($patients as $patientUser)
  @foreach($patientUser->patient as $p)
    <div class="modal fade" id="patientModal{{ $p->id }}" tabindex="-1"
         aria-labelledby="patientModalLabel{{ $p->id }}" aria-hidden="true">
